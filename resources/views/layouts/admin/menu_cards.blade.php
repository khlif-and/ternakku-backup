            <section class="grid grid-cols-1 gap-6 p-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5" style="perspective: 1000px;">
                ...
                <a href="{{ url('select-farm?redirect_url=care-livestock/dashboard') }}"
...
<button
        onclick="event.stopPropagation(); setTimeout(() => window.location.href='{{ route('livestock_outlet.dashboard') }}', 100)"
    class="card-container group relative block w-full transform-gpu text-left transition-all duration-300 ease-out hover:z-10">
    <div class="card-content relative z-10 flex h-full flex-col items-center justify-center rounded-2xl bg-white/80 p-6 text-center backdrop-blur-sm transition-colors duration-300 dark:bg-white/80">
        {{-- Warna diubah dari emerald ke blue --}}
        <div class="mb-4 flex h-16 w-16 items-center justify-center rounded-xl bg-blue-100 shadow-lg shadow-blue-500/10 transition-all duration-300 group-hover:scale-110 group-hover:bg-blue-200 dark:bg-blue-900/50 dark:group-hover:bg-blue-900">
            {{-- Pastikan ikon 'ic_outlet_ternak.png' ada di folder assets Anda --}}
            <img src="{{ asset('home/assets/icons/ic_pelihara_ternak.png') }}" alt="Outlet Ternak" class="h-9 w-9 object-contain transition-transform duration-300 group-hover:scale-95" />
        </div>
        <p class="font-bold text-slate-700 transition-colors duration-300 group-hover:text-blue-700 dark:text-slate-700 dark:group-hover:text-blue-700">
            {{-- Teks diubah --}}
            Outlet Ternak
        </p>
    </div>
