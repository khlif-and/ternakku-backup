                ...
                @forelse($treatments as $index => $item)
                    <tr class="hover:bg-gray-50 transition duration-150 ease-in-out">
                        ...
                        <td class="px-6 py-4 text-sm text-gray-500 align-top">
                            {{-- Obat --}}
                            @if($item->treatmentIndividuMedicineItems->isNotEmpty())
                                <div class="mb-2">
                                    <span class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Obat:</span>
                                    <ul class="list-disc list-inside mt-1">
                                        @foreach($item->treatmentIndividuMedicineItems as $med)
                            ...
                            {{-- Tindakan --}}
                            @if($item->treatmentIndividuTreatmentItems->isNotEmpty())
                                <div>
                                    <span class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Tindakan:</span>
                                    <ul class="list-disc list-inside mt-1">
                                        @foreach($item->treatmentIndividuTreatmentItems as $act)
                                            <li>{{ $act->name }}</li>
                                        @endforeach
                                    </ul>
                                </div>
                            ...
                            @if($item->treatmentIndividuMedicineItems->isEmpty() && $item->treatmentIndividuTreatmentItems->isEmpty())
