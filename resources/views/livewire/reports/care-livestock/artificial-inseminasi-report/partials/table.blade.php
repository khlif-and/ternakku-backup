<div class="bg-white rounded-xl shadow-sm border overflow-hidden">
    @php
        $headers = [
            ...
            ['label' => 'Straw / Semen', 'class' => 'text-left'],
            ['label' => 'Inseminator', 'class' => 'text-left'],
        ];
    @endphp
    
    <div class="p-6">
        <h3 class="text-lg font-bold text-gray-800 mb-4">Riwayat Inseminasi Buatan</h3>
        <x-table.wrapper :headers="$headers">
            @forelse ($inseminations as $item)
                <tr class="hover:bg-gray-50">
                   ...
                   <td class="px-4 py-3">
                        {{ $item->insemination->executor ?? '-' }}
                    </td>
                </tr>
            @empty
                <x-table.empty colspan="6" empty="Tidak ada data inseminasi pada periode ini." />
            @endforelse
        </x-table.wrapper>
