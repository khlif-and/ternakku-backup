<div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
        <h3 class="text-lg font-semibold text-gray-800">Riwayat Analisis Susu</h3>
        <span class="px-3 py-1 bg-emerald-100 text-emerald-800 text-xs font-medium rounded-full">
            Total Record: {{ $data->total() }} Data
        </span>
        <div class="flex space-x-2">
            <a href="{{ route('admin.care-livestock.milk-analysis-global-report.export-pdf', ['farm_id' => $farm->id, 'start_date' => $start_date, 'end_date' => $end_date]) }}"
                target="_blank"
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse($data as $index => $item)
                    <tr class="hover:bg-gray-50 transition duration-150 ease-in-out">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 align-middle text-center">
                            {{ $item->at ? 'Positif' : 'Negatif' }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 align-middle text-center">
                            {{ $item->ab ? 'Positif' : 'Negatif' }}
                        </td>
                    ...
                    <tr>
                        <td colspan="11" class="px-6 py-10 text-center text-gray-500">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-sm text-gray-500 mt-1">Coba ubah filter tanggal.</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
