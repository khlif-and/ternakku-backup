<div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
        <h3 class="text-lg font-semibold text-gray-800">Riwayat Surat Jalan</h3>
        <span class="px-3 py-1 bg-blue-100 text-blue-800 text-xs font-medium rounded-full">
            Total: {{ $data->total() }}
        </span>
        <div class="flex space-x-2">
            @if ($showReport)
                <a href="{{ route('admin.care-livestock.qurban-delivery-order-report.export-pdf', ['farm_id' => $farm->id] + request()->query()) }}"
                    target="_blank"
                ...
                @forelse($data as $item)
                    <tr class="hover:bg-gray-50 transition duration-150 ease-in-out">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 align-top">
                            {{ $item->qurbanCustomerAddress->qurbanCustomer->name ?? '-' }}
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-600 align-top">
                            {{ $item->qurbanCustomerAddress->address ?? '-' }}
                        </td>
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-10 text-center text-gray-500">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-base font-medium text-gray-900">Tidak ada data surat jalan</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
