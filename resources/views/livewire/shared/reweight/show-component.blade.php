    ...
    <x-admin.feature-card title="Detail Penimbangan Ulang" subtitle="Informasi detail data penimbangan">
        ...
        <div class="grid lg:grid-cols-3 gap-6">
            <div class="lg:col-span-1 space-y-6">
                <x-sales.transaction-info :transactionAccount="$reweight->livestockReweightH->transaction_number"
                    :orderDate="$reweight->livestockReweightH->transaction_date"
                    :description="$reweight->livestockReweightH->notes" :editUrl="route('shared.reweight.edit', [$farm->id, $reweight->id])" deleteAction="delete" />
            </div>

            <div class="lg:col-span-2 space-y-6">
                <div class="bg-white rounded-lg shadow-sm border p-6">
                    <h3 class="text-lg font-semibold text-gray-800 border-b pb-4 mb-4">Informasi Ternak & Hasil Timbang
                    </h3>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div class="mt-1 flex items-center gap-3">
                                <div
                                    class="h-10 w-10 bg-gray-200 rounded-full flex items-center justify-center font-bold text-gray-600">
                                    {{ substr($reweight->livestock->eartag, 0, 2) }}
                                </div>
                                <div>
                                    <div class="font-bold text-gray-900">{{ $reweight->livestock->eartag }}</div>
                                    <div class="text-xs text-gray-500">
                                        {{ $reweight->livestock->livestockType->name ?? '-' }}
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    @if($reweight->photo)
                        <div class="mt-6 border-t pt-4">
                            ...
                            <img src="{{ getNeoObject($reweight->photo) }}"
