    ...
    <div class="grid lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-1 space-y-6">
            <div class="bg-white rounded-lg border p-4 space-y-3">
                <div>
                    ...
                    <div class="text-sm font-semibold text-gray-800">
                        {{ $treatmentIndividu->treatmentH?->transaction_date ? date('d M Y', strtotime($treatmentIndividu->treatmentH->transaction_date)) : '-' }}
                    </div>
                </div>

                <div>
                    <div class="text-xs text-gray-500">Ternak (Eartag)</div>
                    <div class="text-sm font-semibold text-gray-800">
                        {{ $treatmentIndividu->livestock?->eartag_number ?? $treatmentIndividu->livestock?->rfid_number ?? '-' }}
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="text-sm">
                        <span class="px-2 py-0.5 bg-red-50 text-red-700 rounded text-xs font-semibold border border-red-100">
                            {{ $treatmentIndividu->disease?->name ?? '-' }}
                        </span>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-lg border p-4">
                <div class="mb-3 font-semibold text-gray-700">Profil Ternak</div>
                <div class="space-y-3">
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-500">Nama:</span>
                        <span class="font-medium">{{ $treatmentIndividu->livestock?->name ?? '-' }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-500">Kandang:</span>
                        <span class="font-medium">{{ $treatmentIndividu->livestock?->pen?->name ?? '-' }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-500">Jenis:</span>
                        <span class="font-medium">{{ $treatmentIndividu->livestock?->livestockType?->name ?? '-' }}</span>
                    </div>
                </div>
            </div>
        </div>
