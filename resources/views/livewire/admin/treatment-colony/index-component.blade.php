<div>
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        ...
        <x-button.link href="{{ route('admin.care-livestock.treatment-colony.create', $farm->id) }}" color="green">
    ...
    <x-table.wrapper :headers="$headers">
        @forelse($items as $index => $item)
            <tr class="hover:bg-gray-50 transition-colors">
                ...
                <td class="px-4 py-3 border-b text-gray-600 text-sm">Rp {{ number_format($item->average_cost, 0, ',', '.') }}</td>
                <td class="px-4 py-3 border-b">
                    <div class="flex items-center justify-center gap-2">
                        <x-button.action href="{{ route('admin.care-livestock.treatment-colony.show', [$farm->id, $item->id]) }}" color="gray">Detail</x-button.action>
                        <x-button.action href="{{ route('admin.care-livestock.treatment-colony.edit', [$farm->id, $item->id]) }}" color="blue">Edit</x-button.action>
        ...
        @empty
            <x-table.empty colspan="8" empty="Belum ada data treatment koloni." />
        @endforelse
    </x-table.wrapper>
</div>