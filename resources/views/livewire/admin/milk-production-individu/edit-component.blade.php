    ...
    <form wire:submit.prevent="save" class="w-full">
        ...
        <div class="bg-white border rounded-lg overflow-hidden mb-8 shadow-sm">
            <div class="px-4 py-3 bg-gray-50 border-b flex justify-between items-center">
                <span class="font-bold text-gray-700 text-xs uppercase tracking-widest">Rincian Produksi Per Ternak</span>
            ...
            <div class="p-4 space-y-4">
                @foreach($items as $index => $item)
                    <div class="flex flex-col md:flex-row items-end gap-4" wire:key="item-{{ $index }}">
                        <div class="flex-[2] w-full">
                            <label class="block text-[10px] font-black text-gray-400 mb-1 uppercase">Pilih Ternak (Betina)</label>
                            <select wire:model="items.{{ $index }}.livestock_id" class="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none bg-white">
        ...
        <x-form.footer 
            backRoute="{{ route('admin.care-livestock.milk-production-individu.index', $farm->id) }}"
