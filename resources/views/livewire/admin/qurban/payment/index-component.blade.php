            ...
            <tr class="hover:bg-gray-50 transition-colors">
                ...
                <td class="px-4 py-3 border-b">
                    <div class="flex items-center justify-center gap-2">
                        <x-button.action href="{{ route('admin.qurban.payment.show', $item->id) }}" color="gray">Detail</x-button.action>
                        <x-button.action href="{{ route('admin.qurban.payment.edit', $item->id) }}" color="blue">Edit</x-button.action>
                        <x-button.primary type="button" wire:click="delete({{ $item->id }})" wire:confirm="Apakah Anda yakin ingin menghapus data pembayaran ini?" color="red" size="sm">Hapus</x-button.primary>
                    </div>
                </td>
            </tr>
        @empty
