<div>
    ...
    <div class="max-w-5xl bg-white rounded-xl border shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b bg-gray-50 flex justify-between items-center">
            ...
            <div
                class="text-[10px] font-black px-3 py-1 {{ $milkAnalysisGlobal->at ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700' }} rounded-full uppercase tracking-widest">
        ...
        <div class="p-8 grid grid-cols-1 md:grid-cols-4 gap-y-8 gap-x-12">
            <div class="space-y-1">
                ...
                <p class="text-sm font-bold text-gray-900">
                    {{ date('d F Y', strtotime($milkAnalysisGlobal->transaction_date)) }}</p>
            </div>

            <div class="space-y-1">
                <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Berat Jenis (BJ)</p>
                <p class="text-lg font-black text-blue-600">{{ number_format($milkAnalysisGlobal->bj, 4, ',', '.') }}
                </p>
            </div>
            ...
            <div class="space-y-1">
                ...
                <p class="text-sm font-bold text-gray-900 uppercase">
                    {{ number_format($milkAnalysisGlobal->a_water, 2, ',', '.') }}%</p>
            </div>

            <div class="space-y-1">
                <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Resazurin (RZN)</p>
                <p class="text-sm font-bold text-gray-900 uppercase">
                    {{ $milkAnalysisGlobal->rzn !== null ? number_format($milkAnalysisGlobal->rzn, 2, ',', '.') : '-' }}
                </p>
            </div>
        ...
        <div class="px-6 py-4 bg-gray-50 border-t flex justify-between items-center">
            <x-button.link href="{{ route('admin.care-livestock.milk-analysis-global.index', $farm->id) }}"
                color="gray">
                KEMBALI
            </x-button.link>
            <div class="flex gap-3">
                <x-button.link
                    href="{{ route('admin.care-livestock.milk-analysis-global.edit', [$farm->id, $milkAnalysisGlobal->id]) }}"
                    color="blue">
                    EDIT DATA
                </x-button.link>
                <button wire:click="delete" wire:confirm="Hapus data analisis permanen?"
