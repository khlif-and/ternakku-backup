    ...
    <x-admin.feature-card title="Detail Pembayaran Qurban" subtitle="Informasi detail pembayaran qurban">
        ...
        <div class="grid lg:grid-cols-3 gap-6">
            <div class="lg:col-span-1 space-y-6">
                <x-sales.transaction-info transactionAccount="-" :orderDate="$payment->transaction_date" description="-"
                    :editUrl="route('admin.qurban.payment.edit', $payment->id)" deleteAction="delete">
                    <div class="mt-4 space-y-3">
                        <div>
                            <div class="text-xs text-gray-500 uppercase font-bold tracking-wider">Ternak</div>
                            <div class="text-sm font-medium text-gray-800">
                                {{ $payment->livestock->eartag_number ?? '-' }}
                                ({{ $payment->livestock->gender ?? '-' }}) -
                                {{ $payment->livestock->livestockBreed->name ?? '-' }}
                            </div>
                        </div>
                ...
                <x-sales.customer-info :name="$payment->qurbanCustomer->user->name ?? '-'"
                    :phone="$payment->qurbanCustomer->user->phone_number ?? '-'"
                ...
                <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
                    ...
                    <div class="p-4">
                        <div class="grid grid-cols-2 gap-4 text-sm">
                            ...
                            <div class="font-medium text-right">{{ $payment->farm->name ?? '-' }}</div>
