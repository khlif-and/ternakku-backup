...
<div class="flex flex-col sm:flex-row items-center justify-between px-8 py-4 gap-4">
    <div>
        <h1 class="text-xl font-bold text-gray-800">Livestock Birth</h1>
        <p class="mt-1 text-sm text-gray-500">Daftar semua data kelahiran ternak di peternakan.</p>
    </div>
    <div class="flex-shrink-0">
        <a href="{{ route('admin.care_livestock.livestock_birth.create', ['farm_id' => $farmId]) }}"
...
<div class="px-8 py-4">
    <form method="GET" action="{{ route('admin.care_livestock.livestock_birth.index', ['farm_id' => $farmId]) }}"
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Tgl Lahir</th>
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Eartag Induk</th>
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Nama Induk</th>
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Perkiraan Sapih</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($births as $index => $birth)
                            @php($lv = optional($birth->reproductionCycle)->livestock)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                <td class="px-6 py-4 text-sm text-gray-500">
                                    {{ ($births->firstItem() ?? 0) + $index }}
                                </td>

                                <td class="px-6 py-4 text-sm text-gray-800">
                                    {{ $birth->transaction_date
                                        ? \Carbon\Carbon::parse($birth->transaction_date)->format('d M Y')
                                        : '-' }}
                                </td>
                                ...
                                <td class="px-6 py-4 text-sm text-gray-800"
                                    title="{{ $birth->notes }}">
                                    {{ \Illuminate\Support\Str::limit($birth->notes, 20, '...') }}
                                </td>

                                <td class="px-6 py-4 text-right text-sm font-medium">
                                    <div class="flex items-center justify-end space-x-2">
                                        <a href="{{ route('admin.care_livestock.livestock_birth.edit', ['farm_id' => $farmId, 'id' => $birth->id]) }}"
                                        ...
                                        <form action="{{ route('admin.care_livestock.livestock_birth.destroy', ['farm_id' => $farmId, 'id' => $birth->id]) }}"
                                              method="POST"
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="text-center py-10 text-gray-500">
                                    Belum ada data kelahiran ternak.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
