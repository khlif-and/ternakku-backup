...
<div class="px-8 py-4 flex items-center justify-between">
    <div>
        <h1 class="text-xl font-bold text-gray-800">Detail Perawatan Individu</h1>
        <p class="mt-1 text-sm text-gray-500">Ringkasan transaksi, ternak, penyakit, serta item obat & tindakan.</p>
    </div>
    <div class="flex items-center gap-2">
        <a href="{{ route('admin.care-livestock.treatment-individu.edit', ['farm_id' => $farmId, 'id' => $treatmentIndividu->id]) }}"
        ...
        <a href="{{ route('admin.care-livestock.treatment-individu.index', ['farm_id' => $farmId]) }}"
        ...
        <div class="bg-white rounded-lg border p-4 space-y-3">
            <div>
                ...
                <div class="text-sm font-semibold text-gray-800">
                    {{ optional($treatmentIndividu->treatmentH)->transaction_date
                        ? \Carbon\Carbon::parse($treatmentIndividu->treatmentH->transaction_date)->format('d M Y')
                        : '-' }}
                </div>
            </div>
            ...
            <div>
                ...
                <div class="text-sm font-semibold text-gray-800">
                    {{
                        optional($treatmentIndividu->livestock)->eartag
                        ?? optional($treatmentIndividu->livestock)->eartag_number
                        ?? optional($treatmentIndividu->livestock)->ear_tag
                        ?? optional($treatmentIndividu->livestock)->tag
                        ?? optional($treatmentIndividu->livestock)->code
                        ?? optional($treatmentIndividu->livestock)->rfid_number
                        ?? '-'
                    }}
                </div>
            </div>
            ...
            <div>
                ...
                <div class="text-sm font-semibold text-gray-800">
                    {{
                        optional($treatmentIndividu->livestock)->name
                        ?? optional($treatmentIndividu->livestock)->nama
                        ?? optional($treatmentIndividu->livestock)->display_name
                        ?? optional($treatmentIndividu->livestock)->nickname
                        ?? '-'
                    }}
                </div>
            </div>
            ...
            <div>
                ...
                <div class="text-sm font-semibold text-gray-800">
                    {{ optional($treatmentIndividu->disease)->name ?? '-' }}
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    ...
                    <div class="text-sm font-semibold text-gray-800">
                        Rp {{ number_format($treatmentIndividu->total_cost ?? 0, 0, ',', '.') }}
                    </div>
                </div>
                <div>
                    <div class="text-xs text-gray-500">Catatan</div>
                    <div class="text-sm text-gray-800">
                        {{ $treatmentIndividu->notes ?: '-' }}
                    </div>
                </div>
            </div>

            <form action="{{ route('admin.care-livestock.treatment-individu.destroy', ['farm_id' => $farmId, 'id' => $treatmentIndividu->id]) }}"
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($treatmentIndividu->treatmentIndividuMedicineItems as $i => $mi)
        ...
        <div class="bg-white rounded-lg border overflow-hidden">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($treatmentIndividu->treatmentIndividuTreatmentItems as $j => $ti)
                            <tr class="hover:bg-gray-50">
                    ...
                    @if(($treatmentIndividu->treatmentIndividuMedicineItems ?? collect())->count()
                        || ($treatmentIndividu->treatmentIndividuTreatmentItems ?? collect())->count())
