...
<div class="flex flex-col sm:flex-row items-center justify-between px-8 py-4 gap-4">
    <div>
        <h1 class="text-xl font-bold text-gray-800">Treatment Individu</h1>
        <p class="mt-1 text-sm text-gray-500">Daftar semua data treatment individu di peternakan.</p>
    </div>
    <div class="flex-shrink-0">
        <a href="{{ route('admin.care-livestock.treatment-individu.create', ['farm_id' => $farmId]) }}"
...
<div class="px-8 py-4">
    <form method="GET" action="{{ route('admin.care-livestock.treatment-individu.index', ['farm_id' => $farmId]) }}"
        ...
        <input type="text" name="search" id="search-input" value="{{ request('search') }}"
               ...
               placeholder="Cari eartag, nama ternak, penyakit...">
    </form>
</div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        @forelse ($items as $index => $fi)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                {{-- Hitung item (butuh withCount di controller) --}}
                                <td class="px-6 py-4 text-sm text-gray-800">
                                    Obat: {{ $fi->treatment_individu_medicine_items_count ?? 0 }}
                                    •
                                    Tindakan: {{ $fi->treatment_individu_treatment_items_count ?? 0 }}
                                </td>
                                ...
                                <td class="px-6 py-4 text-right text-sm font-medium">
                                    <div class="flex items-center justify-end space-x-2">
                                        <a href="{{ route('admin.care-livestock.treatment-individu.edit', ['farm_id' => $farmId, 'id' => $fi->id]) }}"
                                        ...
                                        <form action="{{ route('admin.care-livestock.treatment-individu.destroy', ['farm_id' => $farmId, 'id' => $fi->id]) }}"
                                              method="POST"
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="text-center py-10 text-gray-500">
                                    Belum ada data treatment individu.
                                </td>
                            </tr>
                        @endforelse
