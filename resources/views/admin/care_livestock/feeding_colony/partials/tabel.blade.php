...
<div class="flex flex-col sm:flex-row items-center justify-between px-8 py-4 gap-4">
    <div>
        ...
        <p class="mt-1 text-sm text-gray-500">Daftar semua data pemberian pakan koloni di peternakan.</p>
    </div>
    <div class="flex-shrink-0">
        <a href="{{ route('admin.care-livestock.feeding-colony.create', ['farm_id' => $farmId]) }}"
...
<div class="px-8 py-4">
    <form method="GET" action="{{ route('admin.care-livestock.feeding-colony.index', ['farm_id' => $farmId]) }}"
        ...
        <input type="text" name="search" id="search-input" value="{{ request('search') }}"
               ...
               placeholder="Cari nama/ kode kandang...">
    </form>
</div>
                        ...
                        @forelse ($items as $index => $fc)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-800">
                                    {{ optional($fc->feedingH)->transaction_date
                                        ? \Carbon\Carbon::parse($fc->feedingH->transaction_date)->format('d M Y')
                                        : '-' }}
                                </td>

                                <td class="px-6 py-4 text-sm text-gray-800">
                                    {{ optional($fc->pen)->name
                                        ?? optional($fc->pen)->code
                                        ?? ('Kandang #'.($fc->pen_id ?? '-')) }}
                                </td>

                                <td class="px-6 py-4 text-sm text-gray-800">
                                    {{ $fc->total_livestock ?? 0 }}
                                </td>

                                <td class="px-6 py-4 text-sm text-gray-800">
                                    {{ $fc->feeding_colony_items_count ?? 0 }}
                                </td>

                                <td class="px-6 py-4 text-sm text-gray-800">
                                    Rp {{ number_format($fc->total_cost ?? 0, 0, ',', '.') }}
                                </td>

                                <td class="px-6 py-4 text-sm text-gray-800">
                                    Rp {{ number_format($fc->average_cost ?? 0, 0, ',', '.') }}
                                </td>

                                <td class="px-6 py-4 text-sm text-gray-800" title="{{ $fc->notes }}">
                                    {{ \Illuminate\Support\Str::limit($fc->notes, 20, '...') }}
                                </td>

                                <td class="px-6 py-4 text-right text-sm font-medium">
                                    <div class="flex items-center justify-end space-x-2">
                                        <a href="{{ route('admin.care-livestock.feeding-colony.edit', ['farm_id' => $farmId, 'id' => $fc->id]) }}"
                                        ...
                                        <form action="{{ route('admin.care-livestock.feeding-colony.destroy', ['farm_id' => $farmId, 'id' => $fc->id]) }}"
                                              method="POST"
