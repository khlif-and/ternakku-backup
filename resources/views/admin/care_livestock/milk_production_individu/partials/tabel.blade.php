{{-- resources/views/admin/care_livestock/milk_production_individu/partials/tabel.blade.php --}}

{{-- Header dan Tombol Aksi Utama --}}
<div class="flex flex-col sm:flex-row items-center justify-between px-8 py-4 gap-4">
    <div>
        <h1 class="text-xl font-bold text-gray-800">Produksi Susu Individu</h1>
        <p class="mt-1 text-sm text-gray-500">Daftar semua data Produksi Susu per ternak di peternakan.</p>
    </div>
    <div class="flex-shrink-0">
        <a href="{{ route('admin.care-livestock.milk-production-individu.create', $farm->id) }}"
...
{{-- Filter --}}
<div class="px-8 py-4">
    <form method="GET" action="{{ route('admin.care-livestock.milk-production-individu.index', $farm->id) }}"
            ...
            <select name="livestock_id" id="livestock_id" class="block w-full border border-gray-300 rounded-lg text-sm bg-white text-gray-900 px-3 py-2">
                ...
                @foreach($livestocks as $livestock)
                    <option value="{{ $livestock->id }}" {{ request('livestock_id') == $livestock->id ? 'selected' : '' }}>
                        {{-- DIUBAH: Menggunakan kolom 'eartag_number' --}}
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            @php
                                $headers = [
                                    'No', 'ID Ternak', 'Tanggal', 'Shift', 'Waktu', 'Nama Pemerah',
                                    'Jumlah (L)', 'Kondisi', 'Catatan', 'Aksi',
                                ];
                            @endphp
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($milkProductions as $index => $item)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                <td class="px-4 py-4 text-sm text-gray-500">
                                    {{ $milkProductions->firstItem() + $index }}
                                </td>
                                ...
                                <td class="px-4 py-4 text-right text-sm font-medium">
                                    <div class="flex items-center justify-end space-x-2">
                                        <a href="{{ route('admin.care-livestock.milk-production-individu.edit', [$farm->id, $item->id]) }}"
                                        ...
                                        <form
                                            action="{{ route('admin.care-livestock.milk-production-individu.destroy', [$farm->id, $item->id]) }}"
                                            method="POST"
                        ...
                        @empty
                            <tr>
                                <td colspan="10" class="text-center py-10 text-gray-500">
                                    Belum ada data produksi susu individu.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
...
@if ($milkProductions instanceof \Illuminate\Pagination\LengthAwarePaginator && $milkProductions->count())
<div class="flex flex-col sm:flex-row items-center justify-between gap-4 px-8 py-4">
    <div class="text-sm text-gray-700">
        Menampilkan
        <span class="font-medium">{{ $milkProductions->firstItem() }}</span>
        sampai
        <span class="font-medium">{{ $milkProductions->lastItem() }}</span>
        dari
        <span class="font-medium">{{ $milkProductions->total() }}</span>
        hasil
    </div>
    <div class="flex flex-col sm:flex-row items-center justify-between gap-4 px-8 py-4">
        <div class="flex flex-col sm:flex-row items-center gap-2 w-full sm:w-auto">
            {{ $milkProductions->withQueryString()->links('vendor.pagination.modern-pagination') }}
        </div>
    </div>
</div>
@endif
