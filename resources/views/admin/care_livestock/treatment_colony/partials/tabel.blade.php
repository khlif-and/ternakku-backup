...
<div class="flex flex-col sm:flex-row items-center justify-between px-8 py-4 gap-4">
    <div>
        <h1 class="text-xl font-bold text-gray-800">Treatment Koloni</h1>
        <p class="mt-1 text-sm text-gray-500">Daftar semua data treatment koloni di peternakan.</p>
    </div>
    <div class="flex-shrink-0">
        <a href="{{ route('admin.care-livestock.treatment-colony.create', ['farm_id' => $farmId]) }}"
...
<div class="px-8 py-4">
    <form method="GET" action="{{ route('admin.care-livestock.treatment-colony.index', ['farm_id' => $farmId]) }}"
        ...
        <input type="text" name="search" id="search-input" value="{{ request('search') }}"
               ...
               placeholder="Cari kandang, penyakit...">
    </form>
</div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Kandang</th>
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Penyakit</th>
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Jumlah Ternak</th>
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Item (Obat/Tindakan)</th>
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Rata-rata/Ekor</th>
                        ...
                        @forelse ($items as $index => $fi)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-800">
                                    {{ optional($fi->treatmentH)->transaction_date
                                        ? \Carbon\Carbon::parse($fi->treatmentH->transaction_date)->format('d M Y')
                                        : '-' }}
                                </td>

                                <td class="px-6 py-4 text-sm text-gray-800">
                                    {{ optional($fi->pen)->name ?? '-' }}
                                </td>

                                <td class="px-6 py-4 text-sm text-gray-800">
                                    {{ optional($fi->disease)->name ?? '-' }}
                                </td>

                                <td class="px-6 py-4 text-sm text-gray-800">
                                    {{ number_format($fi->total_livestock ?? 0, 0, ',', '.') }}
                                </td>

                                {{-- Pastikan controller memakai withCount:
                                     ->withCount(['treatmentColonyMedicineItems','treatmentColonyTreatmentItems'])
                                --}}
                                <td class="px-6 py-4 text-sm text-gray-800">
                                    Obat: {{ $fi->treatment_colony_medicine_items_count ?? 0 }}
                                    •
                                    Tindakan: {{ $fi->treatment_colony_treatment_items_count ?? 0 }}
                                </td>

                                <td class="px-6 py-4 text-sm text-gray-800">
                                    Rp {{ number_format($fi->average_cost ?? 0, 0, ',', '.') }}
                                </td>

                                <td class="px-6 py-4 text-sm text-gray-800">
                                    Rp {{ number_format($fi->total_cost ?? 0, 0, ',', '.') }}
                                </td>

                                <td class="px-6 py-4 text-sm text-gray-800" title="{{ $fi->notes }}">
                                    {{ \Illuminate\Support\Str::limit($fi->notes, 20, '...') }}
                                </td>

                                <td class="px-6 py-4 text-right text-sm font-medium">
                                    <div class="flex items-center justify-end space-x-2">
                                        <a href="{{ route('admin.care-livestock.treatment-colony.edit', ['farm_id' => $farmId, 'id' => $fi->id]) }}"
                                        ...
                                        <form action="{{ route('admin.care-livestock.treatment-colony.destroy', ['farm_id' => $farmId, 'id' => $fi->id]) }}"
                                              method="POST"
                        ...
                        @empty
                            <tr>
                                <td colspan="10" class="text-center py-10 text-gray-500">
                                    Belum ada data treatment koloni.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
