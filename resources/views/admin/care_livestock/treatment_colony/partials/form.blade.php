...
<form action="{{ route('admin.care-livestock.treatment-colony.store', ['farm_id' => $farm->id]) }}" method="POST" class="w-full max-w-full">
    @csrf

    {{-- Baris 1: Tanggal transaksi, Pilih kandang, Pilih penyakit --}}
            ...
            <select id="pen_id" name="pen_id"
                ...
                @foreach ($pens as $pen)
                    @php
                        ...
                        $label = ($pen->name ?? 'Tanpa Nama') . ' — Kap: ' . number_format($capacity,0,',','.') .
                                 ', Pop: ' . number_format($population,0,',','.');
                    @endphp
    ...
    <div class="grid md:grid-cols-3 md:gap-6">
        ...
        <div class="mb-8">
            <label class="block mb-2 text-sm font-medium text-gray-700">Populasi Saat Ini</label>
    ...
    <div class="flex items-center justify-between mt-4">
        ...
        {{-- Tidak wajib dikirim; controller akan hitung ulang. Tetap disimpan untuk UX. --}}
...
<script>
    ...
    (function () {
        ...
        const sync = function () {
            ...
            if (cap)  cap.value  = (opt.getAttribute('data-capacity') || '0').toString();
            if (pop)  pop.value  = (opt.getAttribute('data-population') || '0').toString();
        };
    ...
    (function () {
        ...
        theBtnAddTreat = document.getElementById('btnAddTreat');
