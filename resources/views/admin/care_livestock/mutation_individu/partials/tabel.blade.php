...
<div class="flex flex-col sm:flex-row items-center justify-between px-8 py-4 gap-4">
    <div>
        <h1 class="text-xl font-bold text-gray-800">Mutasi Individu</h1>
        <p class="mt-1 text-sm text-gray-500">Daftar semua mutasi individu di peternakan.</p>
    </div>
    <div class="flex-shrink-0">
        <a href="{{ route('admin.care-livestock.mutation-individu.create', ['farm_id' => $farmId]) }}"
...
<div class="px-8 py-4">
    <form method="GET" action="{{ route('admin.care-livestock.mutation-individu.index', ['farm_id' => $farmId]) }}"
        ...
        <input type="text" name="search" id="search-input" value="{{ request('search') }}"
               ...
               placeholder="Cari eartag, nama ternak, kandang...">
    </form>
</div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Dari Kandang</th>
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Ke Kandang</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @if(isset($items) && count($items))
                            @foreach ($items as $index => $mi)
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    <td class="px-6 py-4 text-sm text-gray-500">{{ $index + 1 }}</td>

                                    <td class="px-6 py-4 text-sm text-gray-800">
                                        {{ optional($mi->mutationH)->transaction_date
                                            ? \Carbon\Carbon::parse($mi->mutationH->transaction_date)->format('d M Y')
                                            : '-' }}
                                    </td>

                                    <td class="px-6 py-4 text-sm text-gray-800">
                                        {{
                                            optional($mi->livestock)->eartag
                                            ?? optional($mi->livestock)->eartag_number
                                            ?? optional($mi->livestock)->ear_tag
                                            ?? optional($mi->livestock)->tag
                                            ?? optional($mi->livestock)->code
                                            ?? optional($mi->livestock)->rfid_number
                                            ?? '-'
                                        }}
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-800">
                                        {{
                                            optional($mi->livestock)->name
                                            ?? optional($mi->livestock)->nama
                                            ?? optional($mi->livestock)->display_name
                                            ?? optional($mi->livestock)->nickname
                                            ?? '-'
                                        }}
                                    </td>

                                    <td class="px-6 py-4 text-sm text-gray-800">
                                        {{ optional($mi->penFrom)->name ?? ('#'.$mi->from) }}
                                    </td>

                                    <td class="px-6 py-4 text-sm text-gray-800">
                                        {{ optional($mi->penTo)->name ?? ('#'.$mi->to) }}
                                    </td>

                                    <td class="px-6 py-4 text-sm text-gray-800" title="{{ $mi->notes }}">
                                        {{ \Illuminate\Support\Str::limit($mi->notes, 20, '...') }}
                                    </td>

                                    <td class="px-6 py-4 text-right text-sm font-medium">
                                        <div class="flex items-center justify-end space-x-2">
                                            <a href="{{ route('admin.care-livestock.mutation-individu.edit', ['farm_id' => $farmId, 'id' => $mi->id]) }}"
                                            ...
                                            <form action="{{ route('admin.care-livestock.mutation-individu.destroy', ['farm_id' => $farmId, 'id' => $mi->id]) }}"
                                                  method="POST"
                        ...
                        @else
                            <tr>
                                <td colspan="9" class="text-center py-10 text-gray-500">
                                    Belum ada data mutasi individu.
                                </td>
                            </tr>
                        @endif
                    </tbody>
