...
<div class="flex flex-col sm:flex-row items-center justify-between px-8 py-4 gap-4">
    <div>
        <h1 class="text-xl font-bold text-gray-800">Pregnant Check</h1>
        <p class="mt-1 text-sm text-gray-500">Daftar semua data pregnant check di peternakan.</p>
    </div>
    <div class="flex-shrink-0">
        <a href="{{ route('admin.care_livestock.pregnant_check.create', ['farm_id' => $farmId]) }}"
...
<div class="px-8 py-4">
    <form method="GET" action="{{ route('admin.care_livestock.pregnant_check.index', ['farm_id' => $farmId]) }}"
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Tgl Cek Bunting</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($items as $index => $pc)
                            @php($lv = optional($pc->reproductionCycle)->livestock)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                <td class="px-6 py-4 text-sm text-gray-500">{{ $index + 1 }}</td>

                                {{-- Tanggal dari header --}}
                                <td class="px-6 py-4 text-sm text-gray-800">
                                    {{ optional($pc->pregnantCheck)->transaction_date
                                        ? \Carbon\Carbon::parse($pc->pregnantCheck->transaction_date)->format('d M Y')
                                        : '-' }}
                                </td>
                                ...
                                <td class="px-6 py-4 text-sm text-gray-800">
                                    Rp {{ number_format($pc->cost ?? 0, 0, ',', '.') }}
                                </td>

                                {{-- Catatan dari header --}}
                                <td class="px-6 py-4 text-sm text-gray-800"
                                    title="{{ optional($pc->pregnantCheck)->notes }}">
                                    {{ \Illuminate\Support\Str::limit(optional($pc->pregnantCheck)->notes, 20, '...') }}
                                </td>

                                <td class="px-6 py-4 text-right text-sm font-medium">
                                    <div class="flex items-center justify-end space-x-2">
                                        <a href="{{ route('admin.care_livestock.pregnant_check.edit', ['farm_id' => $farmId, 'id' => $pc->id]) }}"
                                        ...
                                        <form action="{{ route('admin.care_livestock.pregnant_check.destroy', ['farm_id' => $farmId, 'id' => $pc->id]) }}"
                                              method="POST"
