...
<form action="{{ route('admin.care-livestock.livestock-death.store', $farm->id) }}" method="POST"
    enctype="multipart/form-data" class="w-full max-w-full">
    @csrf
        ...
        <div class="mb-8">
            ...
            <select id="livestock_id" name="livestock_id"
                ...
                @foreach ($livestocks as $livestock)
                    <option value="{{ $livestock->id }}" data-eartag="{{ $livestock->eartag ?? '-' }}"
    ...
    <div class="grid md:grid-cols-3 md:gap-6">
        <div class="mb-8">
            ...
            <input id="inputNamaTernak" type="text" value="(Akan terisi otomatis)"
        ...
        <div class="mb-8">
            ...
            <input id="inputJenisTernak" type="text" value="(Akan terisi otomatis)"
                class="bg-gray-200 border border-gray-300 text-sm rounded-lg block w-full p-3 cursor-not-allowed"
                readonly>
        </div>
        <div class="mb-8">
            <label class="block mb-2 text-sm font-medium text-gray-700">Ras</label>
            <input id="inputRasTernak" type="text" value="(Akan terisi otomatis)"
                class="bg-gray-200 border border-gray-300 text-sm rounded-lg block w-full p-3 cursor-not-allowed"
                readonly>
        </div>
    </div>
    ...
    <div class="mb-8">
        <label for="disease_id" class="block mb-2 text-sm font-medium text-gray-700">Penyebab Kematian
            (Penyakit)</label>
    ...
    <div class="mb-8">
        ...
        <input id="indication" type="text" name="indication" value="{{ old('indication') }}"
            class="bg-gray-50 border border-gray-300 text-sm rounded-lg block w-full p-3"
            placeholder="Gejala, tanda, atau ciri-ciri...">
        @error('indication')
            <span class="text-xs text-red-600">{{ $message }}</span>
        @enderror
    </div>
    ...
    <div class="flex justify-end mt-4">
        <button type="submit"
            class="text-white bg-red-600 hover:bg-red-700 font-medium rounded-lg text-base px-8 py-3 transition-all">Simpan
            Data</button>
    </div>
</form>
