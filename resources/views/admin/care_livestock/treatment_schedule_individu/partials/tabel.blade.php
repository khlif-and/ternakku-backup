...
<div class="flex flex-col sm:flex-row items-center justify-between px-8 py-4 gap-4">
    <div>
        <h1 class="text-xl font-bold text-gray-800">Jadwal Treatment Individu</h1>
        <p class="mt-1 text-sm text-gray-500">Daftar semua jadwal treatment individu di peternakan.</p>
    </div>
    <div class="flex-shrink-0">
        <a href="{{ route('admin.care-livestock.treatment-schedule-individu.create', ['farm_id' => $farmId]) }}"
...
<div class="px-8 py-4">
    <form method="GET" action="{{ route('admin.care-livestock.treatment-schedule-individu.index', ['farm_id' => $farmId]) }}"
        ...
        <input type="text" name="search" id="search-input" value="{{ request('search') }}"
               ...
               placeholder="Cari eartag, nama ternak, obat, tindakan...">
    </form>
</div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Tgl Jadwal</th>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Obat (nama • satuan • qty)</th>
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Tindakan</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @if(isset($items) && count($items))
                            @foreach ($items as $index => $si)
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    <td class="px-6 py-4 text-sm text-gray-500">{{ $index + 1 }}</td>

                                    <td class="px-6 py-4 text-sm text-gray-800">
                                        {{ optional($si->treatmentSchedule)->transaction_date
                                            ? \Carbon\Carbon::parse($si->treatmentSchedule->transaction_date)->format('d M Y')
                                            : '-' }}
                                    </td>

                                    <td class="px-6 py-4 text-sm text-gray-800">
                                        {{ $si->schedule_date
                                            ? \Carbon\Carbon::parse($si->schedule_date)->format('d M Y')
                                            : '-' }}
                                    </td>

                                    <td class="px-6 py-4 text-sm text-gray-800">
                                        {{
                                            optional($si->livestock)->eartag
                                            ?? optional($si->livestock)->eartag_number
                                            ?? optional($si->livestock)->ear_tag
                                            ?? optional($si->livestock)->tag
                                            ?? optional($si->livestock)->code
                                            ?? optional($si->livestock)->rfid_number
                                            ?? '-'
                                        }}
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-800">
                                        {{
                                            optional($si->livestock)->name
                                            ?? optional($si->livestock)->nama
                                            ?? optional($si->livestock)->display_name
                                            ?? optional($si->livestock)->nickname
                                            ?? '-'
                                        }}
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-800">
                                        {{ $si->medicine_name ?? '-' }}
                                        @if(($si->medicine_name ?? null) || ($si->medicine_unit ?? null) || !is_null($si->medicine_qty_per_unit))
                                            <span class="text-gray-500">•</span>
                                            {{ $si->medicine_unit ?? '-' }}
                                            <span class="text-gray-500">•</span>
                                            {{ !is_null($si->medicine_qty_per_unit) ? number_format($si->medicine_qty_per_unit, 2, ',', '.') : '-' }}
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-800" title="{{ $si->notes }}">
                                        {{ \Illuminate\Support\Str::limit($si->notes, 20, '...') }}
                                    </td>

                                    <td class="px-6 py-4 text-right text-sm font-medium">
                                        <div class="flex items-center justify-end space-x-2">
                                            <a href="{{ route('admin.care-livestock.treatment-schedule-individu.edit', ['farm_id' => $farmId, 'id' => $si->id]) }}"
                                            ...
                                            <form action="{{ route('admin.care-livestock.treatment-schedule-individu.destroy', ['farm_id' => $farmId, 'id' => $si->id]) }}"
                                                  method="POST"
                        ...
                        @else
                            <tr>
                                <td colspan="9" class="text-center py-10 text-gray-500">
                                    Belum ada jadwal treatment individu.
                                </td>
                            </tr>
                        @endif
                    </tbody>
