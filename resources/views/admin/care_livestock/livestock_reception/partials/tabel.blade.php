...
<div class="flex flex-col sm:flex-row items-center justify-between px-8 py-4 gap-4">
    <div>
        <h1 class="text-xl font-bold text-gray-800">Registrasi Ternak</h1>
        <p class="mt-1 text-sm text-gray-500">Daftar semua ternak yang telah terdaftar di peternakan.</p>
    </div>
...
<div class="px-8 py-4">
    <form method="GET" action="{{ route('admin.care-livestock.livestock-reception.index', $farm->id) }}" class="relative w-full max-w-xs">
                        ...
                        @forelse($receptions as $index => $reception)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
                                    {{ \Carbon\Carbon::parse($reception->livestockReceptionH->transaction_date)->format('d M Y') ?? '-' }}
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    @php
                                        $sex = trim(strtolower($reception->livestockSex->name ?? ''));
                                    @endphp
                                    <span
                                        class="px-2.5 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full
                                        {{ $sex === 'jantan' ? 'bg-green-100 text-green-800' : 'bg-pink-100 text-pink-800' }}">
                                        {{ ucfirst($sex) ?: '-' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
                                    {{ $reception->age_years }} thn, {{ $reception->age_months }} bln
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
                                    {{ $reception->weight }} kg
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ $reception->price_per_head ? 'Rp ' . number_format($reception->price_per_head) : '-' }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ $reception->supplier ?? '-' }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500"
                                    ...
                                    {{ \Illuminate\Support\Str::limit($reception->notes, 20, '...') }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end space-x-2">
                                        ...
                                        <form
                                            action="{{ route('admin.care-livestock.livestock-reception.destroy', [$farm->id, $reception->id]) }}"
                                            method="POST"
                            ...
                            <tr>
                                <td colspan="13" class="text-center py-10">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="mt-4 text-sm font-semibold text-gray-700">Data Tidak Ditemukan</p>
                                        <p class="text-sm text-gray-500">Belum ada data registrasi ternak yang tersedia.
                                        </p>
                                    </div>
                                </td>
                            </tr>
...
<div class="flex flex-col sm:flex-row items-center justify-between gap-4 px-8 py-4">
    <div class="text-sm text-gray-700">
        Menampilkan
        <span class="font-medium">{{ $receptions->firstItem() ?? 0 }}</span>
        sampai
        <span class="font-medium">{{ $receptions->lastItem() ?? 0 }}</span>
        dari
        <span class="font-medium">{{ $receptions->total() }}</span>
        hasil
    </div>

    <div class="flex flex-col sm:flex-row items-center justify-between gap-4 px-8 py-4">
        <div class="flex flex-col sm:flex-row items-center gap-2 w-full sm:w-auto">
            {{ $receptions->links('vendor.pagination.modern-pagination') }}
        </div>
    </div>
</div>
