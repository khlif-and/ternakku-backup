...
<div class="flex flex-col sm:flex-row items-center justify-between px-8 py-4 gap-4">
    <div>
        <h1 class="text-xl font-bold text-gray-800">Pemberian Pakan Individu</h1>
        <p class="mt-1 text-sm text-gray-500">Daftar semua data pemberian pakan individu di peternakan.</p>
    </div>
    <div class="flex-shrink-0">
        <a href="{{ route('admin.care-livestock.feeding-individu.create', ['farm_id' => $farmId]) }}"
           class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg px-4 py-2 text-sm shadow-sm transition-all">
            ...
            Tambah Pemberian Pakan Individu
        </a>
    </div>
</div>
...
<div class="px-8 py-4">
    <form method="GET" action="{{ route('admin.care-livestock.feeding-individu.index', ['farm_id' => $farmId]) }}"
        ...
        <input type="text" name="search" id="search-input" value="{{ request('search') }}"
               ...
               placeholder="Cari berdasarkan eartag, nama ternak...">
    </form>
</div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Ras</th>
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Total Item Pakan</th>
                        ...
                        @forelse ($items as $index => $fi)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-800">
                                    {{ optional($fi->feedingH)->transaction_date
                                        ? \Carbon\Carbon::parse($fi->feedingH->transaction_date)->format('d M Y')
                                        : '-' }}
                                </td>
                                ...
                                <td class="px-6 py-4 text-sm text-gray-800">
                                    {{
                                        optional($fi->livestock)->eartag
                                        ?? optional($fi->livestock)->eartag_number
                                        ?? optional($fi->livestock)->ear_tag
                                        ?? optional($fi->livestock)->tag
                                        ?? optional($fi->livestock)->code
                                        ?? optional($fi->livestock)->rfid_number
                                        ?? '-'
                                    }}
                                </td>
                                ...
                                <td class="px-6 py-4 text-sm text-gray-800">
                                    {{
                                        optional($fi->livestock)->name
                                        ?? optional($fi->livestock)->nama
                                        ?? optional($fi->livestock)->display_name
                                        ?? optional($fi->livestock)->nickname
                                        ?? '-'
                                    }}
                                </td>

                                <td class="px-6 py-4 text-sm text-gray-800">
                                    {{ optional(optional($fi->livestock)->livestockType)->name ?? '-' }}
                                </td>

                                <td class="px-6 py-4 text-sm text-gray-800">
                                    {{ optional(optional($fi->livestock)->livestockBreed)->name ?? '-' }}
                                </td>

                                {{-- Hitung item pakan dari withCount --}}
                                <td class="px-6 py-4 text-sm text-gray-800">
                                    {{ $fi->feeding_individu_items_count ?? 0 }}
                                </td>
                                ...
                                <td class="px-6 py-4 text-right text-sm font-medium">
                                    <div class="flex items-center justify-end space-x-2">
                                        <a href="{{ route('admin.care-livestock.feeding-individu.edit', ['farm_id' => $farmId, 'id' => $fi->id]) }}"
                                        ...
                                        <form action="{{ route('admin.care-livestock.feeding-individu.destroy', ['farm_id' => $farmId, 'id' => $fi->id]) }}"
                                              method="POST"
                        ...
                        @empty
                            <tr>
                                <td colspan="10" class="text-center py-10 text-gray-500">
                                    Belum ada data pemberian pakan individu.
                                </td>
                            </tr>
                        @endforelse
