...
<div class="flex flex-col sm:flex-row items-center justify-between px-8 py-4 gap-4">
    <div>
        <h1 class="text-xl font-bold text-gray-800">Analisis Susu Global</h1>
        <p class="mt-1 text-sm text-gray-500">Daftar hasil analisis susu global di peternakan.</p>
    </div>
    <div class="flex-shrink-0">
        <a href="{{ route('admin.care-livestock.milk-analysis-global.create', $farm->id) }}"
...
{{-- Filter Tanggal --}}
<div class="px-8 py-4">
    <form method="GET" action="{{ route('admin.care-livestock.milk-analysis-global.index', $farm->id) }}"
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            @php
                                $headers = [
                                    'No', 'Tanggal', 'BJ', 'AT', 'AB', 'MBRT', 'Air (%)', 'Protein (%)', 'Lemak (%)', 'SNF', 'TS', 'RZN', 'Catatan', 'Aksi'
                                ];
                            @endphp
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($data as $index => $item)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                <td class="px-4 py-4 text-sm text-gray-500">{{ $data->firstItem() + $index }}</td>
                                ...
                                <td class="px-4 py-4 text-right text-sm font-medium">
                                    <div class="flex items-center justify-end space-x-2">
                                        <a href="{{ route('admin.care-livestock.milk-analysis-global.edit', [$farm->id, $item->id]) }}"
                                        ...
                                        <form
                                            action="{{ route('admin.care-livestock.milk-analysis-global.destroy', [$farm->id, $item->id]) }}"
                                            method="POST"
                        ...
                        @empty
                            <tr>
                                <td colspan="14" class="text-center py-10 text-gray-500">
                                    Belum ada data analisis susu global.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
...
@if ($data instanceof \Illuminate\Pagination\LengthAwarePaginator && $data->count())
<div class="flex flex-col sm:flex-row items-center justify-between gap-4 px-8 py-4">
    ...
    <div class="flex flex-col sm:flex-row items-center justify-between gap-4 px-8 py-4">
        <div class="flex flex-col sm:flex-row items-center gap-2 w-full sm:w-auto">
            {{ $data->links('vendor.pagination.modern-pagination') }}
        </div>
    </div>
</div>
@endif
