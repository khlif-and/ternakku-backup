...
@section('content')
    <div class="p-6">
        <div class="mb-6">
            <p class="text-gray-700 text-lg mb-3 font-semibold">[ Data Timbang Ulang ]</p>
        ...
        <div class="bg-white rounded-2xl shadow-lg overflow-hidden w-full transition-all duration-300">
            ...
            <div class="px-16 py-8">
                ...
                <form action="{{ url('qurban/weight') }}" method="POST" enctype="multipart/form-data" autocomplete="off" class="w-full max-w-full">
                    @csrf

                    <div class="mb-8">
                        <label for="weight" class="block mb-2 text-base font-semibold text-gray-700">Berat (kg)</label>
                        <input type="number" step="0.01"
                            class="w-full px-4 py-3 border rounded-md text-base outline-none focus:ring-2 focus:ring-blue-300 @error('weight') border-red-500 @enderror"
                            id="weight" name="weight" required
                            placeholder="Masukkan berat hewan dalam kg">
                        @error('weight')
                    ...
                    <div class="mb-8 grid grid-cols-1 md:grid-cols-3 gap-8">
                        <div>
                            <label for="pbb" class="block mb-2 text-base font-semibold text-gray-700">PBB (kg)</label>
                            <input type="number" step="0.01"
                                class="w-full px-4 py-3 border rounded-md text-base outline-none focus:ring-2 focus:ring-blue-300 @error('pbb') border-red-500 @enderror"
                                id="pbb" name="pbb" required placeholder="Contoh: 350.5">
                            @error('pbb')
                                <span class="text-red-500 text-xs">{{ $message }}</span>
                            @enderror
                        </div>
                        <div>
                            <label for="dof" class="block mb-2 text-base font-semibold text-gray-700">DOF (Hari)</label>
                            <input type="number"
                                class="w-full px-4 py-3 border rounded-md text-base outline-none focus:ring-2 focus:ring-blue-300 @error('dof') border-red-500 @enderror"
                                id="dof" name="dof" required placeholder="Contoh: 120">
                            @error('dof')
                                <span class="text-red-500 text-xs">{{ $message }}</span>
                            @enderror
                        </div>
                        <div>
                            <label for="adg" class="block mb-2 text-base font-semibold text-gray-700">ADG (kg/hari)</label>
                            <input type="number" step="0.01"
                                class="w-full px-4 py-3 border rounded-md text-base outline-none focus:ring-2 focus:ring-blue-300 @error('adg') border-red-500 @enderror"
                                id="adg" name="adg" required placeholder="Contoh: 1.25">
                            @error('adg')
                                <span class="text-red-500 text-xs">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="mb-8">
                        ...
                        <textarea
                            class="w-full px-4 py-3 border rounded-md text-base outline-none focus:ring-2 focus:ring-blue-300 @error('note') border-red-500 @enderror"
                            id="note" name="note" rows="3" placeholder="Tuliskan catatan tambahan"></textarea>
                        @error('note')
                            <span class="text-red-500 text-xs">{{ $message }}</span>
                        @enderror
                    </div>
    ...
    @enderror
                        ...
                        <div>
                            ...
                            <div id="photo-preview-container" class="mt-4">
                                <img id="photo-preview" src="#" alt="Preview Foto Timbang Ulang" class="hidden w-[140px] h-auto rounded-lg border shadow" />
                            </div>
                        </div>
                    </div>
...
@section('script')
    ...
    <script>
        // Untuk format hari dan bulan Indonesia
        ...
        // Script preview foto (biar tetap jalan)
