    ...
    <div class="p-6">
        ...
        <div id="fleet-list-container"
            ...
            <div class="w-full flex items-center justify-end px-8 h-[90px] bg-white border-b border-gray-200">
                <a href="{{ url('qurban/fleet/create') }}"
            ...
            <div class="overflow-x-auto mt-6 px-8 pb-8">
                <table class="w-full text-center rounded-xl border border-black border-collapse">
                    <thead>
                        <tr>
                            @foreach (['Nama', 'No Polisi', 'Foto', 'Aksi'] as $header)
                    ...
                    <tbody>
                        @forelse($fleets as $fleet)
                            <tr>
                                <td class="py-6 pl-6 border border-black text-left">{{ $fleet->name }}</td>
                                <td class="py-6 pl-6 border border-black text-left">{{ $fleet->police_number }}</td>
                                <td class="py-6 pl-6 border border-black text-left">
                                    @if($fleet->photo)
                                        <img src="{{ getNeoObject($fleet->photo) }}" alt="Fleet Photo" class="rounded-lg shadow" style="width: 100px; height: auto;">
                                    @endif
                                </td>
                                <td class="py-6 pl-6 border border-black text-left">
                                    <div class="flex space-x-2">
                                        {{-- EDIT --}}
                                        <a href="{{ url('qurban/fleet/' . $fleet->id . '/edit') }}"
                                        ...
                                        {{-- DELETE --}}
                                        <form action="{{ url('qurban/fleet/' . $fleet->id) }}" method="post" class="d-inline"
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="py-6 border border-black text-center text-gray-500">Tidak ada data armada</td>
                            </tr>
                        @endforelse
                    </tbody>
                    <thead>
                    ...
                    <div class="pl-2 pb-2 md:pb-0">
                        <span class="text-gray-500 text-base">
                            Showing <span class="font-medium text-gray-700">1</span> to <span
                                class="font-medium text-gray-700">{{ $fleets->count() }}</span> of <span
                                class="font-medium text-gray-700">{{ $fleets->count() }}</span> entries
                        </span>
                    </div>
...
@section('script')
    ...
    <script>
        $(document).ready(function() {
            ...
            setTimeout(function() {
                $('#fleet-list-container').removeClass('opacity-0 translate-y-4')
