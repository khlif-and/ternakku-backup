    ...
    <div class="p-6">
        ...
        <div id="customer-list-container"
            ...
            <div class="w-full flex items-center justify-end px-8 h-[90px] bg-white border-b border-gray-200">
                <a href="{{ url('qurban/customer/create') }}"
            ...
            <div class="overflow-x-auto mt-6 px-8 pb-8">
                <table class="w-full text-center rounded-xl border border-black border-collapse">
                    <thead>
                        <tr>
                            @foreach (['Nama', 'No HP', 'Aksi'] as $header)
                    ...
                    <tbody>
                        @forelse($customers as $customer)
                            <tr>
                                <td class="py-6 pl-6 border border-black text-left">{{ $customer->name }}</td>
                                <td class="py-6 pl-6 border border-black text-left">{{ $customer->phone_number }}</td>
                                <td class="py-6 pl-6 border border-black text-left">
                                    <div class="flex space-x-2">
                                        {{-- EDIT --}}
                                        <a href="{{ url('qurban/customer/' . $customer->id . '/edit') }}"
                                        ...
                                        {{-- DELETE --}}
                                        <form action="{{ url('qurban/customer/' . $customer->id) }}" method="post" class="d-inline"
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="py-6 border border-black text-center text-gray-500">Tidak ada data pelanggan</td>
                            </tr>
                        @endforelse
                    </tbody>
                    <thead>
                    ...
                    <div class="pl-2 pb-2 md:pb-0">
                        <span class="text-gray-500 text-base">
                            Showing <span class="font-medium text-gray-700">1</span> to <span
                                class="font-medium text-gray-700">{{ $customers->count() }}</span> of <span
                                class="font-medium text-gray-700">{{ $customers->count() }}</span> entries
                        </span>
                    </div>
...
@section('script')
    ...
    <script>
        $(document).ready(function() {
            ...
            setTimeout(function() {
                $('#customer-list-container').removeClass('opacity-0 translate-y-4')
