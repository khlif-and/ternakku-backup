        ...
        <main class="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            ...
            <div class="mb-12 text-center">
                ...
                <div class="mt-4 flex justify-center gap-3">
                    <div
                        class="inline-block bg-white text-blue-700 text-sm font-semibold px-4 py-2 rounded-full border border-slate-200">
                        🐄 Kandang Saat Ini: <span
                            class="font-bold text-slate-800">{{ \App\Models\Farm::find(session('selected_farm'))?->name ?? 'Belum dipilih' }}</span>
                    </div>
                    ...
                    @php
                        ...
                        if ($farmId) {
                            $fu = \App\Models\FarmUser::where('user_id', auth()->id())->where('farm_id', $farmId)->first();
                            if ($fu) {
                                $userRole = $fu->farm_role;
                            } elseif (\App\Models\Farm::where('id', $farmId)->where('owner_id', auth()->id())->exists()) {
                                $userRole = 'OWNER';
                            }
                        }
                    @endphp
                    ...
                    <div
                        class="inline-block bg-white text-emerald-700 text-sm font-semibold px-4 py-2 rounded-full border border-slate-200">
                        👤 Role: <span class="font-bold text-slate-800">{{ $userRole }}</span>
                    </div>
                </div>
            </div>
                ...
                <div class="lg:col-span-1">
                    ...
                    <div class="p-6 rounded-2xl shadow-lg bg-white/70 backdrop-blur-lg border border-black/5">
                        <div class="flex flex-col space-y-3">
                            ...
                            <x-menu.shortcut-link href="#" label="Tambah Penjualan Ternak"
