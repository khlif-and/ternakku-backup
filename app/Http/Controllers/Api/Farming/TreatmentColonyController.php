...
use App\Http\Resources\Farming\TreatmentColonyResource;
...
class TreatmentColonyController extends Controller
{
    public function store(TreatmentColonyStoreRequest $request, $farmId)
    {
        ...
        try {
            ...
            return ResponseHelper::success(new TreatmentColonyResource($treatmentColonyD), 'Data created successfully', 200);
    ...
    public function index($farmId, Request $request)
    {
        ...
        $data = TreatmentColonyResource::collection($treatmentColony->get());

        $message = $treatmentColony->count() > 0 ? 'Data retrieved successfully' : 'No Data found';
    ...
    public function show($farmId, $treatmentColonyId)
    {
        ...
        return ResponseHelper::success(new TreatmentColonyResource($treatmentColony), 'Data retrieved successfully');
    }
    ...
    public function update(TreatmentColonyUpdateRequest $request, $farmId , $treatmentColonyId)
    {
        ...
        try {
            ...
            return ResponseHelper::success(new TreatmentColonyResource($treatmentColonyD), 'Data updated successfully');
    ...
    public function destroy($farmId, $treatmentColonyId)
    {
        ...
        // Cari treatmentColonyD dengan memastikan farm dan tipe Colony
        $treatmentColonyD = TreatmentColonyD::whereHas('treatmentH', function ($query) use ($farm) {
            $query->where('farm_id', $farm->id)->where('type', 'colony');
        })->findOrFail($treatmentColonyId);
        ...
        try {
            ...
            $treatmentH = $treatmentColonyD->treatmentH;
            if (!$treatmentH->treatmentColonyD()->exists()) {
                $treatmentH->delete();
            }
