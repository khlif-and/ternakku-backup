...
class ArtificialInseminationController extends Controller
{
    ...
    public function index($farmId , Request $request) : JsonResponse
    {
        ...
        $inseminationArtificial = InseminationArtificial::whereHas('insemination', function ($query) use ($farm, $request) {
        ...
        if ($request->filled('livestock_type_id')) {
            $inseminationArtificial->whereHas('reproductionCycle.livestock', function ($query) use ($request) {
        ...
        $data = ArtificialInseminationResource::collection($inseminationArtificial->get());
