...
public function store(Request $request)
{
    ...
    if (!\App\Models\Region::where('id', $regionId)->exists()) {
        ...
        while (!$regionData && $hasMore) {
            $response = \Illuminate\Support\Facades\Http::get('https://feedmill.ternakku.com/api/data-master/region', [
                'name' => '',
                'per_page' => 1000,
                'page' => $page,
            ]);
    ...
    try {
        ...
        \Illuminate\Support\Facades\Log::info('🚜 FarmDetail Incoming Data', [
            'farm_id'     => $farm->id,
        ...
        \Illuminate\Support\Facades\Log::error('❌ Gagal simpan FarmDetail', [
            'error' => $e->getMessage(),
            'line'  => $e->getLine(),
            'file'  => $e->getFile(),
        ]);
