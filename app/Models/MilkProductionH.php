...
class MilkProductionH extends Model
{
    use HasFactory;

    protected $table = 'milk_production_h';
    ...
    private function generateTransactionNumber($type , $transactionDate, $farmId)
    {
        ...
        $prefix = $type == 'colony' ? "$year$month-MPC-" : "$year$month-MPI-";
    ...
    public function milkProductionIndividuD()
    {
        return $this->hasMany(MilkProductionIndividuD::class, 'milk_production_h_id');
    }

    public function milkProductionColonyD()
    {
        return $this->hasMany(MilkProductionColonyD::class, 'milk_production_h_id');
    }
}
